 ...
 * 절기 기준으로 구분된 월별 간지
 */
const MONTHLY_GANZHI_2026: Array<{
  month: number;
  ganZhi: string;
  ganHan: string;
  jiHan: string;
 ...
 * 현재 대운 찾기 (2026년 기준)
 */
function getCurrentDaeun(
  sajuData: SajuData,
): { period: any; index: number } | null {
 ...
 * 재성 확인 (일간 기준)
 */
function getWealthStars(dayMaster: string): {
  jeongjae: string;
  pyeonjae: string;
} {
 ...
 * 관성 확인 (일간 기준)
 */
function getOfficialStars(dayMaster: string): {
  jeonggwan: string;
  pyeongwan: string;
} {
  const dayElement = getOhaeng(dayMaster);
  ...
  const yangOfficial: Record<
    Element,
    { jeonggwan: string; pyeongwan: string }
  > = {
...
export class NewYearFortune2026Calculator {
    ...
    const analysis = generateTotalFortuneText(
      {
        ...
        keywords,
        analysis: "",
        advice: { firstHalf: "", secondHalf: "", direction, color },
      },
      seunTenGodsGan,
      seunTenGodsJi,
      strength,
   ...
   * 문서 3.2.2 알고리즘
   */
  private static calculateWealthFortune(
    seun: Seun,
    sajuData: SajuData,
  ): FortuneCategory2026 {
    const dayMaster = sajuData.day.ganHan;
    ...
    const hasWealthInWonguk =
      sajuData.year.tenGodsGan === "정재" ||
      sajuData.year.tenGodsGan === "편재" ||
      sajuData.month.tenGodsGan === "정재" ||
      sajuData.month.tenGodsGan === "편재" ||
      sajuData.day.tenGodsGan === "정재" ||
      sajuData.day.tenGodsGan === "편재" ||
      sajuData.hour.tenGodsGan === "정재" ||
      sajuData.hour.tenGodsGan === "편재";

    // STEP 4: 신강/신약에 따른 재성 해석
    // STEP 5: 식상생재 여부 확인
    const hasSiksin =
      sajuData.year.tenGodsGan === "식신" ||
      sajuData.year.tenGodsGan === "상관" ||
      sajuData.month.tenGodsGan === "식신" ||
      sajuData.month.tenGodsGan === "상관";
   ...
   * 문서 3.3.2 알고리즘
   */
  private static calculateLoveFortune(
    seun: Seun,
    sajuData: SajuData,
  ): FortuneCategory2026 {
    const dayMaster = sajuData.day.ganHan;
    ...
    if (gender === "male") {
      ...
      spouseStar =
        seun.ganHan === officialStars.jeonggwan
          ? officialStars.jeonggwan
          : seun.ganHan === officialStars.pyeongwan
            ? officialStars.pyeongwan
            : null;
    }
    ...
    const dayRelationship = SeunFortuneCalculator.analyzeJiRelationship(
      seun.jiHan,
      dayJi,
    );
   ...
   * 문서 3.4.2 알고리즘
   */
  private static calculateCareerFortune(
    seun: Seun,
    sajuData: SajuData,
  ): FortuneCategory2026 {
    const dayMaster = sajuData.day.ganHan;
    ...
    const isOfficialStar =
      seun.tenGodsGan === "정관" || seun.tenGodsGan === "편관";
    ...
    const hasOfficialInWonguk =
      sajuData.year.tenGodsGan === "정관" ||
      sajuData.year.tenGodsGan === "편관" ||
      sajuData.month.tenGodsGan === "정관" ||
      sajuData.month.tenGodsGan === "편관";
   ...
   * 문서 3.5.3 알고리즘
   */
  private static calculateHealthFortune(
    seun: Seun,
    sajuData: SajuData,
  ): FortuneCategory2026 {
    // STEP 1: 원국 오행 과불급 체크
    ...
    const jiRelationships = SeunFortuneCalculator.analyzeSeun(seun, sajuData);
    const hasChung =
      jiRelationships.year?.type === "충" ||
      jiRelationships.month?.type === "충" ||
      jiRelationships.day?.type === "충";
    const hasHyung =
      jiRelationships.year?.type === "형" ||
   ...
   * 문서 3.6.2 알고리즘
   */
  private static calculateMonthlyFortunes(
    seun: Seun,
    sajuData: SajuData,
  ): MonthlyFortune2026[] {
    const dayMaster = sajuData.day.ganHan;
    ...
    return MONTHLY_GANZHI_2026.map((monthData) => {
      ...
      // STEP 2: 월운과 원국 작용
      const monthRelationship = SeunFortuneCalculator.analyzeJiRelationship(
        monthData.jiHan,
        dayJi,
      );

      // STEP 3: 월운과 세운 조합
      const monthSeunRelationship = SeunFortuneCalculator.analyzeJiRelationship(
        monthData.jiHan,
        seun.jiHan,
      );
      ...
      // 세운 조합
      if (
        monthSeunRelationship.type === "합" ||
        monthSeunRelationship.type === "반합"
      )
        score += 0.3;
      ...
      // 각 항목별 점수 계산
      const wealthScore = this.calculateMonthlyWealthScore(
        monthTenGodsGan,
        monthTenGodsJi,
        monthRelationship,
      ...
      const loveScore = this.calculateMonthlyLoveScore(
        monthTenGodsGan,
        monthRelationship,
        monthData.jiHan,
        sajuData,
      ...
      const careerScore = this.calculateMonthlyCareerScore(
        monthTenGodsGan,
        monthTenGodsJi,
        monthRelationship,
        monthData.jiHan,
      ...
      const healthScore = this.calculateMonthlyHealthScore(
        monthData.ganHan,
        monthData.jiHan,
        monthRelationship,
        monthSeunRelationship,
      ...
      if (monthTenGodsGan === "편재" || monthTenGodsGan === "정재")
        keywords.push("재물");
      if (monthTenGodsGan === "편관" || monthTenGodsGan === "정관")
        keywords.push("직장");
      if (monthTenGodsGan === "편인" || monthTenGodsGan === "정인")
        keywords.push("학습");
      if (monthTenGodsGan === "식신" || monthTenGodsGan === "상관")
        keywords.push("능력");
   ...
   * 월별 재물운 점수 계산
   */
  private static calculateMonthlyWealthScore(
    ...
    >,
    monthSeunRelationship: ReturnType<
      typeof SeunFortuneCalculator.analyzeJiRelationship
    >,
    sajuData: SajuData,
    strength: "strong" | "weak" | "neutral",
    ...
    // 월운 십신이 재성인지 확인
    const isMonthWealth =
      monthTenGodsGan === "정재" || monthTenGodsGan === "편재";

    if (isMonthWealth) {
      wealthScore += 1.0;

      // 신강/신약에 따른 해석
      if (strength === "strong") wealthScore += 0.3; // 신강 + 재성 = 통제 가능
      if (strength === "weak") wealthScore -= 0.3; // 신약 + 재성 = 부담

      // 식상생재 확인 (원국에 식상이 있고 월운이 재성)
      const hasSiksinInWonguk =
        sajuData.year.tenGodsGan === "식신" ||
        sajuData.year.tenGodsGan === "상관" ||
        sajuData.month.tenGodsGan === "식신" ||
        sajuData.month.tenGodsGan === "상관" ||
        sajuData.day.tenGodsGan === "식신" ||
        sajuData.day.tenGodsGan === "상관" ||
        sajuData.hour.tenGodsGan === "식신" ||
        sajuData.hour.tenGodsGan === "상관";

      if (hasSiksinInWonguk) wealthScore += 0.5; // 식상생재
    }
    ...
    if (hasWealthInWonguk) wealthScore += 0.3;

    // 월운-원국 작용
    if (monthRelationship.type === "합" || monthRelationship.type === "반합") {
      wealthScore += 0.3; // 합 = 재물 기회 증가
    }
    if (monthRelationship.type === "충") {
      wealthScore -= 0.5; // 충 = 재물 변동
    }
    ...
    return Math.min(5, Math.max(1, wealthScore));
  }

  /**
   * 월별 애정운 점수 계산
   */
  private static calculateMonthlyLoveScore(
    monthTenGodsGan: string,
    monthRelationship: ReturnType<
      typeof SeunFortuneCalculator.analyzeJiRelationship
    >,
    ...
    if (gender === "male") {
      ...
      // 여성: 관성 = 배우자성
      spouseStar =
        monthTenGodsGan === "정관" || monthTenGodsGan === "편관"
          ? monthTenGodsGan
          : null;
    }

    if (spouseStar) loveScore += 1.0;

    // 월운과 일지(배우자궁) 작용
    if (monthRelationship.type === "합" || monthRelationship.type === "반합") {
      loveScore += 0.5; // 합 = 인연 기회
    }
    if (monthRelationship.type === "충") {
      loveScore -= 0.5; // 충 = 관계 변동
    }

    // 도화살 확인 (월운 지지가 일지의 도화인지)
    const dohwaSals = getDohwaSals(dayJi);
    if (dohwaSals.includes(monthJiHan)) {
      loveScore += 0.5; // 도화 = 이성 매력
    }

    // 원국에 배우자성이 있는지 확인
    let hasSpouseInWonguk = false;
    if (gender === "male") {
      hasSpouseInWonguk =
        sajuData.year.tenGodsGan === "정재" ||
        sajuData.year.tenGodsGan === "편재" ||
      ...
      hasSpouseInWonguk =
        ...
        sajuData.month.tenGodsGan === "정관" ||
        sajuData.month.tenGodsGan === "편관" ||
        sajuData.day.tenGodsGan === "정관" ||
        sajuData.day.tenGodsGan === "편관" ||
        sajuData.hour.tenGodsGan === "정관" ||
        sajuData.hour.tenGodsGan === "편관";
    }

    if (hasSpouseInWonguk) loveScore += 0.3;

    return Math.min(5, Math.max(1, loveScore));
  }

  /**
   * 월별 직장운 점수 계산
   */
  private static calculateMonthlyCareerScore(
    monthTenGodsGan: string,
    monthTenGodsJi: string,
    monthRelationship: ReturnType<
      typeof SeunFortuneCalculator.analyzeJiRelationship
    ...
    const isOfficialStar =
      monthTenGodsGan === "정관" || monthTenGodsGan === "편관";
    const isInStar = monthTenGodsGan === "정인" || monthTenGodsGan === "편인";
    const isSiksin = monthTenGodsGan === "식신" || monthTenGodsGan === "상관";

    if (isOfficialStar) {
      careerScore += 1.0;
      if (strength === "strong") careerScore += 0.3; // 신강 + 관성
      if (strength === "weak") careerScore -= 0.3; // 신약 + 관성
    }
    if (isInStar) careerScore += 0.5; // 인성 = 자격증, 학습
    if (isSiksin) careerScore += 0.5; // 식상 = 능력 발휘
    ...
    if (hasOfficialInWonguk) careerScore += 0.3;

    // 역마살 확인 (월운 지지가 역마인지)
    const yukmaSals = ["寅", "申", "巳", "亥"];
    if (yukmaSals.includes(monthJiHan)) {
      careerScore += 0.3; // 역마 = 이동, 변화
    }
    ...
    if (monthRelationship.type === "충") {
      careerScore -= 0.3; // 충 = 직장 변동
    }

    return Math.min(5, Math.max(1, careerScore));
  }

  /**
   * 월별 건강운 점수 계산
   */
  private static calculateMonthlyHealthScore(
    monthGanHan: string,
    monthJiHan: string,
    monthRelationship: ReturnType<
      typeof SeunFortuneCalculator.analyzeJiRelationship
    ...
    // 월운의 오행 확인
    const monthGanElement = getOhaeng(monthGanHan);
    const monthJiElement = getOhaeng(monthJiHan);

    // 원국 오행 과불급 체크
    const excessElements = sajuData.ohaengAnalysis.excess;
    const missingElements = sajuData.ohaengAnalysis.missing;

    // 월운이 원국 과다 오행을 강화하는지
    if (monthGanElement) {
      const ganElementName = this.getElementKoreanName(monthGanElement);
      if (excessElements.includes(ganElementName)) {
        healthScore -= 0.8; // 과다 오행 강화 = 건강 악화
      }
    }
    if (monthJiElement) {
      const jiElementName = this.getElementKoreanName(monthJiElement);
      if (excessElements.includes(jiElementName)) {
        healthScore -= 0.8;
      }
    }
    ...
    if (monthGanElement) {
      const ganElementName = this.getElementKoreanName(monthGanElement);
      if (missingElements.includes(ganElementName)) {
        healthScore += 0.5; // 부족 오행 보완 = 건강 개선
      }
    }
    if (monthJiElement) {
      const jiElementName = this.getElementKoreanName(monthJiElement);
      if (missingElements.includes(jiElementName)) {
        healthScore += 0.5;
      }
    }
    ...
    // 월운-원국 작용
    if (monthRelationship.type === "충") {
      healthScore -= 0.8; // 충 = 건강 변동, 주의
    }
    if (monthRelationship.type === "형") {
      healthScore -= 0.5; // 형 = 스트레스, 주의
    }
    if (monthRelationship.type === "합" || monthRelationship.type === "반합") {
      healthScore += 0.2; // 합 = 안정
    }

    // 월운-세운 조합
    if (monthSeunRelationship.type === "충") {
      healthScore -= 0.3;
    }
    if (monthSeunRelationship.type === "형") {
      healthScore -= 0.2;
    }

    return Math.min(5, Math.max(1, healthScore));
  }

  /**
   * 오행을 한국어 이름으로 변환
   */
  private static getElementKoreanName(element: Element): string {
