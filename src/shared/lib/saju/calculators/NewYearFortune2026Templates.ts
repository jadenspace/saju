...
import { NewYearFortune2026, FortuneCategory2026, MonthlyFortune2026, SajuData, Seun } from '../../../../entities/saju/model/types';
...
function isPositiveSentence(sentence: string): boolean {
  const positiveKeywords = ['좋', '유리', '기회', '증가', '상승', '발전', '성과', '성취', '안정', '늘어나', '많아', '기대', '좋아져'];
  const negativeKeywords = ['주의', '경고', '피해야', '어려움', '변수', '갈등', '문제', '소진', '방전'];
  
  const hasPositive = positiveKeywords.some(keyword => sentence.includes(keyword));
 ...
 */
function getConnector(prevPositive: boolean, nextPositive: boolean, isEmphasis: boolean = false): string {
...
export function generateWealthFortuneText(
  ...
  // p2: 신강/신약
  const p2Key = strength === 'strong' ? '신강' : strength === 'weak' ? '신약' : null;
  ...
  if (p2Text) {
    const connector = getConnector(isPositiveSentence(p1Text), isPositiveSentence(p2Text));
    text += ` ${connector} ${p2Text}`;
  }
  
  if (p3Text) {
    const prevText = p2Text || p1Text;
    const connector = getConnector(isPositiveSentence(prevText), isPositiveSentence(p3Text), p3Key === '합');
...
export function generateLoveFortuneText(
  ...
  dayRelationship: ReturnType<typeof SeunFortuneCalculator.analyzeJiRelationship>,
  hasDohwa: boolean,
  gender: 'male' | 'female',
  sajuData: SajuData,
  seun: Seun
...
export function generateCareerFortuneText(
  ...
  if (p3Text) {
    const prevText = p2Text || p1Text;
    const connector = getConnector(isPositiveSentence(prevText), isPositiveSentence(p3Text), p3Key === '길');
    text += ` ${connector} ${p3Text}`;
  }
 ...
 */
function isHeeshinForCareer(sipsin: Sipsin, strength: 'strong' | 'weak' | 'neutral'): boolean {
...
export function generateHealthFortuneText(
  ...
  const hasChungOrHyung = hasChung || hasHyung || 
    jiRelationships.day?.type === '충' || jiRelationships.day?.type === '형';
  ...
  if (p3Text) {
    const prevText = p2Text || p1Text;
    const connector = getConnector(isPositiveSentence(prevText), isPositiveSentence(p3Text), p3Key === '평범');
    text += ` ${connector} ${p3Text}`;
  }
...
export function generateMonthlyFortuneText(
  ...
  monthRelationship: ReturnType<typeof SeunFortuneCalculator.analyzeJiRelationship>
