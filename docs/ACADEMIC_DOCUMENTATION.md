...
- 입춘(立春): 1월 → 인월(寅月) (양력 2월 4일경)
- 경칩(驚蟄): 2월 → 묘월(卯月) (양력 3월 6일경)
- 청명(清明): 3월 → 진월(辰月) (양력 4월 5일경)
- 입하(立夏): 4월 → 사월(巳月) (양력 5월 6일경)
- 망종(芒種): 5월 → 오월(午月) (양력 6월 6일경)
- 소서(小暑): 6월 → 미월(未月) (양력 7월 7일경)
- 입추(立秋): 7월 → 신월(申月) (양력 8월 8일경)
- 백로(白露): 8월 → 유월(酉月) (양력 9월 8일경)
- 한로(寒露): 9월 → 술월(戌月) (양력 10월 8일경)
- 입동(立冬): 10월 → 해월(亥月) (양력 11월 8일경)
- 대설(大雪): 11월 → 자월(子月) (양력 12월 7일경)
- 소한(小寒): 12월 → 축월(丑月) (양력 1월 6일경)
...
### 4.4 야자시(夜子時) 및 조자시(朝子時) 처리

현대 사주 계산에서는 23:00~00:00 사이의 시간을 처리하는 방식에 따라 결과가 달라진다. 본 시스템은 두 가지 모드를 지원한다.

1.  **야자시 미적용 (Late Midnight Mode - 기본)**:
    - 23:00를 기점으로 날짜와 일주(日柱)가 다음 날로 바뀐다고 간주한다.
    - 23:00~01:00은 모두 다음 날의 자시(子時)로 처리된다.
2.  **야자시 적용 (Early Midnight Mode)**:
    - 23:00~00:00(야자시)은 **당일**의 일간(日干)을 사용하되, 시간(時干)만 자시로 계산한다.
    - 00:00~01:00(조자시)은 **다음 날**의 일간과 자시를 사용한다.

**코드 구현 논리:**
```typescript
if (midnightMode === 'early' && current.hour === 23) {
  // 당일 일주 유지, 시간만 자시로 재계산
  dayGanZhi = noonLunar.getDayInGanZhiExact();
  timeGanZhi = calculateRatHourGanZhi(dayStem);
}
```
...
### 5.4 음양(陰陽) 판별의 용(用)적용
사주 계산 시 지지의 음양은 체(體, 본체)가 아닌 **용(用, 쓰임)**을 기준으로 한다.

- **子(자)**: 본체는 양(陽)이나, 쓰임은 음(陰)으로 본다.
- **午(오)**: 본체는 양(陽)이나, 쓰임은 음(陰)으로 본다.
- **巳(사)**: 본체는 음(陰)이나, 쓰임은 양(陽)으로 본다.
- **亥(해)**: 본체는 음(陰)이나, 쓰임은 양(陽)으로 본다.

본 시스템의 `getPolarity` 함수는 이를 반영하여 십성을 산출한다.
...
**전통 공식:**
```
대운수(세) = ⌊ |출생시각 - 절기시각| (일 단위) / 3 ⌋
```
*공식 정밀화*: 일수 계산 시 소수점까지 포함하여 계산한 후, 3으로 나눈 최종 결과값에 내림(floor)을 적용한다.
```
...
3. **일수 계산:**
   ...
   function calculateDaeunStartAge(
     ...
     // 3. 대운수 = ⌊일수 / 3⌋ (내림)
     const daeunSu = Math.floor(days / 3);
...
대운의 3일 = 1세 공식은 전통 명리학에서 관행적으로 사용되는 산법이다 (천문학적 정밀 근거라기보다는 상징적 비율로 이해함):
- 음력 1개월 ≈ 30일
- 1년 = 12개월 ≈ 360일
- 360일 / 120세(인간 수명) = 3일
...
### 8.1 한국 표준시 변천사 및 보정 logic

한국의 표준시는 역사적으로 변경되었으므로, 입력된 지방시를 표준 KST(UTC+9)로 정규화한 후 계산해야 한다.

**1. 표준시 정규화 (Normalization to UTC+9):**
- **1908.04.01 ~ 1911.12.31**: UTC+08:30 사용 → **+30분** 보정
- **1954.03.21 ~ 1961.08.09**: UTC+08:30 사용 → **+30분** 보정
- **서머타임(DST)** 적용 시: **-60분** 보정

**2. 진태양시(True Solar Time) 보정:**
- 사용자의 출생지 경도(longitude)를 기준으로 보정한다. (Standard: 135˚E)
- **보정 공식**: `offsetMinutes = (longitude - 135) * 4`
- **서울 기준**: 정규화된 시간에서 **-30분** 차감 (127.5˚E 가정 시).

### 8.2 세운(歲運) 계산 공식

세운은 매년 바뀌는 운으로, 입춘(立春)을 기점으로 한 해의 간지가 결정된다.

**산출 데이터:**
- 해당 연도의 천간/지지 오행 및 색상
- 일간(日干) 대비 연도 간지의 십성(十神)
...
### 9.1 오행 분포 계산
사주 8글자(또는 시간 미상 시 6글자)의 천간/지지 오행 개수를 합산한다. 옵션에 따라 지장간(hidden stems)의 가중치를 포함하여 계산할 수 있다.

**가중치 예시:**
- 천간/지지(본신): 1.0
- 지장간(본기): 1.0 (또는 정규화 비율)
- 지장간(중기): 0.3
- 지장간(여기): 0.1

### 9.2 오행 에너지 수준 (Level)
개수에 따라 다음과 같이 5단계로 분류하여 해석한다.

| 개수 | 수준 | 설명 |
|------|------|------|
| 0개 | 결핍 | 해당 기운이 없어 기능이 정체됨 |
| 1개 | 부족 | 기운이 약하여 보완이 필요함 |
| 2개 | 균형 | 가장 이상적이고 안정적인 상태 |
| 3개 | 강함 | 해당 기운의 특징이 뚜렷하게 나타남 |
| 4개 이상 | 과다 | 기운이 넘쳐 오히려 부작용 발생 가능 |

### 9.3 종합 해석 기준
- **강한 기운 (Excess)**: 3개 이상의 오행
- **부족한 기운 (Deficient)**: 1개의 오행
- **없는 기운 (Missing)**: 0개의 오행
